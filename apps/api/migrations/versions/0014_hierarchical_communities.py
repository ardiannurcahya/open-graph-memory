...
def upgrade() -> None:
    ...
    op.add_column(
        "graph_analytics_runs", sa.Column("algorithm_version", sa.String(100), nullable=True)
    )
    op.add_column("graph_analytics_runs", sa.Column("config", postgresql.JSONB(), nullable=True))
    op.execute(
        "UPDATE graph_analytics_runs SET levels = 1, algorithm_version = 'louvain-v1', config = '{}'::jsonb WHERE levels IS NULL"
    )
    ...
    op.create_check_constraint(
        "ck_analytics_membership_level", "graph_analytics_memberships", "level BETWEEN 0 AND 2"
    )
    op.drop_constraint(
        "graph_analytics_memberships_pkey", "graph_analytics_memberships", type_="primary"
    )
    op.create_primary_key(
        "graph_analytics_memberships_pkey",
        "graph_analytics_memberships",
        ["run_id", "entity_id", "level"],
    )
    op.add_column("graph_analytics_communities", sa.Column("level", sa.Integer(), nullable=True))
    op.add_column(
        "graph_analytics_communities",
        sa.Column("parent_community_id", sa.String(32), nullable=True),
    )
    op.add_column(
        "graph_analytics_communities", sa.Column("internal_edges", sa.Integer(), nullable=True)
    )
    op.add_column(
        "graph_analytics_communities", sa.Column("external_edges", sa.Integer(), nullable=True)
    )
    ...
    op.execute(
        "UPDATE graph_analytics_communities SET level = 0, internal_edges = 0, external_edges = 0, density = 0, importance = 0 WHERE level IS NULL"
    )
    for column in ("level", "internal_edges", "external_edges", "density", "importance"):
        op.alter_column("graph_analytics_communities", column, nullable=False)
    op.drop_constraint(
        "graph_analytics_communities_pkey", "graph_analytics_communities", type_="primary"
    )
    op.create_primary_key(
        "graph_analytics_communities_pkey",
        "graph_analytics_communities",
        ["run_id", "community_id", "level"],
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "ck_analytics_membership_level", "graph_analytics_memberships", type_="check"
    )
