...
class Neo4jGraphStore:
    ...
    async def traverse(
        ...
        one_hop = "CALL { WITH seed MATCH (seed)<-[:SOURCE|TARGET]-(r1:Relation {project_id: $project_id, dataset_id: $dataset_id}) WHERE r1.review_state <> 'rejected' MATCH (r1)-[:SOURCE]->(source1:Entity {project_id: $project_id, dataset_id: $dataset_id}) MATCH (r1)-[:TARGET]->(target1:Entity {project_id: $project_id, dataset_id: $dataset_id}) WITH seed, r1, CASE WHEN source1.id = seed.id THEN target1 ELSE source1 END AS entity1 ORDER BY r1.id LIMIT $fanout RETURN r1, entity1 } MATCH (r1)-[support1:SUPPORTED_BY]->(e1:Evidence {project_id: $project_id, dataset_id: $dataset_id}) RETURN e1.chunk_id AS chunk_id, [e1.chunk_id] AS evidence_chunk_ids, [seed.id, r1.id, entity1.id] AS path, [seed.id, entity1.id] AS entity_ids, [r1.id] AS relation_ids, e1.confidence AS confidence "
        two_hop = "CALL { WITH seed MATCH (seed)<-[:SOURCE|TARGET]-(r1:Relation {project_id: $project_id, dataset_id: $dataset_id}) WHERE r1.review_state <> 'rejected' MATCH (r1)-[:SOURCE]->(source1:Entity {project_id: $project_id, dataset_id: $dataset_id}) MATCH (r1)-[:TARGET]->(target1:Entity {project_id: $project_id, dataset_id: $dataset_id}) WITH seed, r1, CASE WHEN source1.id = seed.id THEN target1 ELSE source1 END AS entity1 ORDER BY r1.id LIMIT $fanout RETURN r1, entity1 } CALL { WITH seed, r1, entity1 MATCH (entity1)<-[:SOURCE|TARGET]-(r2:Relation {project_id: $project_id, dataset_id: $dataset_id}) WHERE r1.id <> r2.id AND r2.review_state <> 'rejected' MATCH (r2)-[:SOURCE]->(source2:Entity {project_id: $project_id, dataset_id: $dataset_id}) MATCH (r2)-[:TARGET]->(target2:Entity {project_id: $project_id, dataset_id: $dataset_id}) WITH seed, r1, entity1, r2, CASE WHEN source2.id = entity1.id THEN target2 ELSE source2 END AS entity2 ORDER BY r2.id LIMIT $fanout RETURN r2, entity2 } MATCH (r1)-[support1:SUPPORTED_BY]->(e1:Evidence {project_id: $project_id, dataset_id: $dataset_id}) MATCH (r2)-[support2:SUPPORTED_BY]->(e2:Evidence {project_id: $project_id, dataset_id: $dataset_id}) RETURN e2.chunk_id AS chunk_id, [e1.chunk_id, e2.chunk_id] AS evidence_chunk_ids, [seed.id, r1.id, entity1.id, r2.id, entity2.id] AS path, [seed.id, entity1.id, entity2.id] AS entity_ids, [r1.id, r2.id] AS relation_ids, (e1.confidence + e2.confidence) / 2 AS confidence "
