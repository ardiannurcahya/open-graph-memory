...
def analyze_graph(entity_ids: list[str], relations: list[tuple[str, str, float]]) -> AnalyticsResult:
    ...
    weighted_degree = {
        entity_id: float(graph.degree(entity_id, weight="weight")) for entity_id in sorted(graph.nodes)
    }
    ...
    return AnalyticsResult(digest, communities, degree, weighted_degree, importance, community_sizes, len(relations))
...
async def refresh_dataset_analytics(
    ...
    entities = list(
        await db.scalars(
            ...
            .where(CanonicalEntity.project_id == project_id, CanonicalEntity.dataset_id == dataset_id)
    ...
    for entity_id in entities:
        ...
        db.add(GraphAnalyticsMembership(run_id=run.id, entity_id=entity_id, community_id=result.communities[entity_id]))
